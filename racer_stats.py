...
st.header("Racer Speed does not seem to correlate to numbeer of races they win")
x = st.slider('How Many Racers to Show',1,len(df_racer))

st.write("Racers by Speed")
df_fastest_Racers = df_racer[['Character', 'Speed']].sort_values("Speed", ascending=False.iloc[x])
